...
class ScanMerger(Node):
    ...
    def _merge_by_binning(self, scans: List[LaserScan]) -> Optional[LaserScan]:
        ring_inc = self._derive_ring_inc(scans)
        ...
        stamp = self._choose_stamp(scans)
        out = LaserScan()
        ...
        ref = min(scans, key=lambda s: s.angle_increment)
