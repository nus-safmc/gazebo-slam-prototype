...
# PX4 SITL (kill the px4 binary itself; otherwise subsequent runs may fail with
# "PX4 server already running for instance 0").
pkill -INT -f '[p]x4_sitl_default/bin/px4' 2>/dev/null || true
sleep 0.2
pkill -TERM -f '[p]x4_sitl_default/bin/px4' 2>/dev/null || true
sleep 0.2
pkill -KILL -f '[p]x4_sitl_default/bin/px4' 2>/dev/null || true

# Sometimes the `make px4_sitl ...` wrapper outlives launch; kill it too.
pkill -f '[m]ake px4_sitl' 2>/dev/null || true
...
# IMPORTANT: don't match "nav2_" in our own launch filename (e.g. px4_nav2_explore.launch.py),
# otherwise this script can kill itself.
...
pkill -f 'nav2_bringup.*launch\.py' 2>/dev/null || true
